...
http.createServer(async function(request, response) {
	let url = request.url;
	if (url == "/") {
		const index = fs.readFileSync("/laboratory-work-9/app/index.html");
		response.writeHead(200, {"Content-Type": "text/html; charset=utf8"});
		response.end(index);
	}
	else if (url == "/style.css") {
		const style = fs.readFileSync("/laboratory-work-9/app/style.css");
		response.writeHead(200, {"Content-Type": "text/css; charset=utf8"});
		response.end(style);
	}
	else if (url == "/main.js") {
		const script = fs.readFileSync("/laboratory-work-9/app/pages/main.js");
...
}).listen(4444, "127.0.0.1", function() {
	console.log("Сервер начал прослушивание запросов на порту 4444");
});
